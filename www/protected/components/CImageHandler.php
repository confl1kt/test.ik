...
class CImageHandler extends CApplicationComponent
{
    ...
    public function watermark($watermarkFile, $offsetX, $offsetY, $corner = self::CORNER_RIGHT_BOTTOM)
    {
        ...
        if ($wImg = $this->loadImage($watermarkFile))
        {
    		...
    		$percent = $this->width*100/400;
            (isset($percent) && strcmp($percent,"") != 0 ) ? $percent = $percent : $percent = 100;
            ...
            $src = imagecreatefrompng($watermarkFile);
            $trans_colour = imagecolorallocatealpha($wImg['image'], 0, 0, 0, 127);
            ...
            switch ($corner)
            {
                ...
                case self::CORNER_RIGHT_TOP:
                    $posX = $this->width - $newwidth - $offsetX;
                    $posY = $offsetY;
                    break;
                case self::CORNER_LEFT_BOTTOM:
                    $posX = $offsetX;
                    $posY = $this->height - $newheight - $offsetY;
                    break;
                ...
                case self::CORNER_CENTER:
                    $posX = floor(($this->width - $newwidth) / 2);
                    $posY = floor(($this->height -$newheight) / 2);
                    break;
                default:
            ...
            imagecopy($this->image, $thumb, $posX, $posY, 0, 0,$newwidth, $newheight);
